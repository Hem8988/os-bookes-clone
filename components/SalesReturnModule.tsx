...
export const SalesReturnModule: React.FC<SalesReturnModuleProps> = ({
  ...
  const [returnReason, setReturnReason] = useState<ReturnDocument['returnReason']>('Wrong Item');
  ...
  useEffect(() => {
    if (returnToEdit) {
      const matched = customers.find((c) => c.name === returnToEdit.partyName);
  ...
  const handleSearchOriginal = () => {
    const match = sales.find(
      (inv) => inv.invoiceNumber.toLowerCase() === originalInvSearch.trim().toLowerCase()
    );
    if (!match) {
      if (originalInvSearch.trim()) alert('No Sales Invoice found with that Invoice No.');
      return;
    }
  ...
  const handleSave = () => {
    ...
    const ret: ReturnDocument = {
      id: returnToEdit?.id || `sret-${Date.now()}`,
      docNumber: returnToEdit?.docNumber || `CN-${new Date().getFullYear()}-${Math.floor(100 + Math.random() * 900)}`,
  ...
  const handlePaymentStatusSave = (
    ...
    if (returnToEdit) {
      if (onUpdateSalesReturn) onUpdateSalesReturn(finalized);
      setIsPaymentStatusOpen(false);
      setPendingReturn(null);
      onClose();
    } else {
      onAddSalesReturn(finalized);
  ...
  return (
    <div className="bg-white dark:bg-slate-900 flex flex-col min-h-[calc(100vh-4rem)] -mx-4 -mt-4 md:-mx-6 md:-mt-6 -mb-24 md:-mb-24">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-teal-600 dark:bg-teal-800">
        <h2 className="text-base font-bold text-white">{returnToEdit ? 'Edit Sales Return' : 'Sales Return'}</h2>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              />
              <button type="button" title="Load Original Sales Invoice" onClick={handleSearchOriginal} className="p-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white">
