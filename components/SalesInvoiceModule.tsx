...
export const SalesInvoiceModule: React.FC<SalesInvoiceModuleProps> = ({
  ...
  useEffect(() => {
    if (invoiceToEdit) {
      const matched = customers.find((c) => c.id === invoiceToEdit.customerId);
      if (matched) setCustomerId(matched.id);
      setInvoiceNoSearch(invoiceToEdit.invoiceNumber);
      setDate(invoiceToEdit.date);
      setPaymentMode(invoiceToEdit.paymentMode === 'Cash' ? 'Cash' : 'Credit');
      setRemark(invoiceToEdit.notes || '');
  ...
  const handleAddRow = () => {
    ...
    const newItem: InvoiceItem = {
      ...
      unitPrice: parseFloat(entryPriceExclTax.toFixed(2)),
      mrp: entry.mrp,
      discountPercent: 0,
      taxRate: entry.gstRate,
      taxableAmount: parseFloat(entryTaxable.toFixed(2)),
      cgstAmount: parseFloat((entryGstAmount / 2).toFixed(2)),
      sgstAmount: parseFloat((entryGstAmount / 2).toFixed(2)),
      igstAmount: 0,
      totalAmount: parseFloat(entryAmount.toFixed(2)),
    };
    setItems([...items, newItem]);
    setEntry(emptyEntry(products));
  };

  const handleEditRow = (item: InvoiceItem) => {
    setEntry({
  ...
  const totals = useMemo(() => {
    ...
    const itemsAmount = items.reduce((sum, item) => sum + item.totalAmount, 0);
    const disOnListPrice = items.reduce(
      (sum, item) => sum + Math.max(0, (item.mrp || 0) - item.unitPrice) * item.quantity,
      0
    );
  ...
  const handleSearchInvoice = () => {
    const match = sales.find(
      (inv) => inv.invoiceNumber.toLowerCase() === invoiceNoSearch.trim().toLowerCase()
    );
    if (!match) {
      if (invoiceNoSearch.trim()) alert('No Sales Invoice found with that Invoice No.');
      return;
    }
    const matched = customers.find((c) => c.id === match.customerId);
    if (matched) setCustomerId(matched.id);
    setDate(match.date);
    setPaymentMode(match.paymentMode === 'Cash' ? 'Cash' : 'Credit');
    setRemark(match.notes || '');
  ...
  const handleSave = () => {
    ...
    const inv: Invoice = {
      ...
      invoiceNumber: invoiceToEdit?.invoiceNumber || `OS-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`,
      date,
      dueDate: invoiceToEdit?.dueDate || new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0],
  ...
  const handlePaymentStatusSave = (
    ...
    if (invoiceToEdit) {
      if (onUpdateInvoice) onUpdateInvoice(pendingInvoice);
      setIsPaymentStatusOpen(false);
      setPendingInvoice(null);
      onClose();
    } else {
      onAddInvoice(pendingInvoice);
      setLastSavedTotal(pendingInvoice.grandTotal);
      setIsPaymentStatusOpen(false);
      setPendingInvoice(null);
      handleResetForm();
    }
    ...
    switch (target) {
      ...
      case 'company-purchase': {
        onAddPurchase({
          ...
          vendorInvoiceNumber: invoiceNoSearch || `INV-REF-${Date.now().toString().slice(-6)}`,
          vendorName: customer.name,
          vendorGstin: customer.gstin || '',
          date,
          subTotal: parseFloat(totals.taxable.toFixed(2)),
      ...
      case 'company-purchase-return': {
        onAddPurchaseReturn({
          ...
          originalInvNumber: invoiceToEdit?.invoiceNumber || invoiceNoSearch || 'N/A',
  ...
  return (
    <div className="bg-white dark:bg-slate-900 flex flex-col min-h-[calc(100vh-4rem)] -mx-4 -mt-4 md:-mx-6 md:-mt-6 -mb-24 md:-mb-24">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-teal-600 dark:bg-teal-800">
        <h2 className="text-base font-bold text-white">{invoiceToEdit ? 'Edit Sales Invoice' : 'Sales Invoice'}</h2>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                placeholder="Search existing invoice number"
              ...
              />
              <button type="button" title="Load Sales Invoice" onClick={handleSearchInvoice} className="p-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            ...
            {items.map((item, idx) => (
              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                ...
                <td className="px-2 py-2.5 text-center font-bold text-emerald-600 dark:text-emerald-400">@{item.taxRate}%</td>
                ...
                <td className="px-2 py-2.5 text-right font-mono">₹{(item.mrp || 0).toLocaleString('en-IN')}</td>
                <td className="px-2 py-2.5 text-right font-mono">₹{item.unitPrice.toLocaleString('en-IN')}</td>
                <td className="px-2 py-2.5 text-right font-mono text-[11px] text-slate-500">Excl.</td>
                <td className="px-2 py-2.5 text-right font-mono font-black">₹{item.totalAmount.toLocaleString('en-IN')}</td>
              ...
              <>
                ...
                <div className="absolute z-20 bottom-full mb-2 right-0 w-64 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl overflow-hidden">
                  ...
                  {CONVERT_OPTIONS.map((opt) => {
                    const Icon = opt.icon;
                    const isCurrentType = opt.key === 'customer-sale';
                    return (
                      <button
                        key={opt.key}
                        type="button"
