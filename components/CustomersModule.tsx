...
export const CustomersModule: React.FC<CustomersModuleProps> = ({ 
  ...
  const filteredCustomers = useMemo(() => {
    return safeCustomers.filter((c) => {
      ...
      const customerCode = String(c.customerCode || c.id || '');
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            {isVendorOnly
              ...
              : isCustomerOnly
              ? 'Manage B2B LPG Commercial Buyers, Customer IDs, Monthly Base Rates & Outstanding Balances'
              : 'Manage B2B LPG Customers, Vendors, GSTIN Registrations, Customer IDs, Monthly Base Rates & Balances'
            }
          </p>
        </div>

        <div className="flex items-center gap-2 flex-wrap">
          {/* Monthly Customer Rates Excel Manager Button */}
          <button
            onClick={() => setIsRateModalOpen(true)}
            className="flex items-center gap-2 py-2.5 px-3.5 rounded-xl bg-teal-50 dark:bg-teal-950/40 hover:bg-teal-100 dark:hover:bg-teal-900/50 text-teal-800 dark:text-teal-200 border border-teal-200 dark:border-teal-800 font-bold text-xs shadow-xs transition-all active:scale-95 cursor-pointer"
            title="Manage monthly customer rates & export/import Excel sheet"
          >
            <FileSpreadsheet className="h-4 w-4 text-teal-600 dark:text-teal-400" />
            <span className="hidden sm:inline">Monthly Rates (Excel)</span>
          </button>
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-mono text-[10px] font-black text-teal-800 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/50 px-2 py-0.5 rounded-md border border-teal-200 dark:border-teal-800">
                                {c.customerCode || c.id}
                              </span>
                              <button
                              ...
                              {Array.isArray(c.partyRates) && c.partyRates.length > 0 && (
                                <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded-md border border-emerald-200 dark:border-emerald-800" title={`${c.partyRates.length} custom rates defined`}>
                                  ★ {c.partyRates.length} Custom Rates
                                </span>
                              )}
                            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        {c.partyRates && c.partyRates.length > 0 && (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-300 font-bold text-[10px] border border-amber-200 dark:border-amber-800">
                            🏷️ {c.partyRates.length} Custom Rate{c.partyRates.length > 1 ? 's' : ''}
                          </span>
                        )}
                      </div>
      ...
      {/* Monthly Customer Rates Master & Excel Sync Modal */}
      <MonthlyCustomerRateModal
        ...
        onUpdateCustomers={(updatedList) => {
          if (onUpdateCustomers) {
            ...
            updatedList.forEach(c => onUpdateCustomer(c));
          }
        }}
      />
